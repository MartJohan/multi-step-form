...
<template>
    <div v-if="step" :class="[step.selectedStep.value > 1 ? 'justify-between' : 'justify-end']"
        ...
        <div class="p-2" :class="[step.selectedStep.value > 1 ? 'block' : 'hidden']">
            <button class="rounded p-2 text-coolGray self-start" @click="step?.previousStep">
                Go Back
            </button>
        </div>
        <div class="p-2">
            <button class="rounded p-2 text-white hover:cursor-pointer font-semibold" :disabled="step.disableNext.value"
