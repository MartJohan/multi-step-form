...
<template>
  <div class="flex flex-col">
    <div
      class="bg-magnolia flex flex-col rounded-lg divide-y divide-coolGray divide-opacity-50 p-2"
    >
      <div class="flex flex-row justify-between text-sm">
        ...
        <p class="p-2 text-marineBlue font-semibold">
          {{
            selectedMonthlyPlan.selectedMonthlyPlan.value
              ? selectedPlan.selectedPlan.value.monthlyPrice + "/mo"
      ...
      >
        <SummaryViewAddons
          ...
          :key="addon.id"
          :addon="addon"
          :show-monthly="selectedMonthlyPlan.selectedMonthlyPlan.value"
        />
        <p
          v-if="!selectedAddons.selectedAddons.value.length"
